...
%        by default.)
%
%        NOTE ON DEPENDENCIES: A dependency analysis will indicate that the
%        Computer Vision System Toolbox is required. I use functionality in
%        that Toolbox (in export function) IF it is on the user license. It
%        is NOT OTHERWISE REQUIRED!
%
%        CURRENT TAB PANELS:
%             EDGE, THRESHOLDING, HOUGH LINE/CIRCLE;
...
% BEGIN NESTED SUBFUNCTIONS
	...
	function exportBW(varargin)
		img = get(imhandles(workingax),'cdata');
		vn = createUniqueName('segtoolimage');
		assignin('base',vn,img);
		throwComment(sprintf('Current image written to %s, and reproduction steps written to Command Window.\n',vn),1,1);
		fprintf('\nCurrent image written to %s\n',vn)
		fprintf('\nREPRODUCTION STEPS:\n******************\n')
